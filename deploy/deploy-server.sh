...
if grep -Eq '^GEMINI_MODEL=gemini-flash-latest$' "$REMOTE_DIR/.env"; then
  sed -i 's/^GEMINI_MODEL=gemini-flash-latest$/GEMINI_MODEL=gemini-3.5-flash-lite/' "$REMOTE_DIR/.env"
elif ! grep -Eq '^GEMINI_MODEL=.*$' "$REMOTE_DIR/.env"; then
  echo "GEMINI_MODEL=gemini-3.5-flash-lite" >> "$REMOTE_DIR/.env"
fi
