...
export class AdminService {
  ...
  async getDashboardKpis() {
    ...
    const [
      ...
      prisma.action.count({
        where: {
          status: { in: [ActionStatus.CANCELLED, ActionStatus.FAILED] },
        },
      }),
  ...
  async getProviders(filters?: {
    ...
    return providers.map((p) => {
      ...
      return {
        ...
        activeLocationsCount: p.locations.filter(
  ...
  async getLiveInspectorLogs(filters: {
    ...
    const createdAt =
      from || to
        ...
        : undefined;
    const providerWhere = filters.providerSlug
      ? { provider: { slug: filters.providerSlug } }
      : undefined;

    const [integrationLogs, webhookLogs, totalIntegration, totalWebhooks] =
      await Promise.all([
        !filters.source || filters.source === 'INTEGRATION'
          ...
          : [],
        !filters.source || filters.source === 'WEBHOOK'
  ...
  async getUnmetDemandAnalytics(filters?: {
    from?: string;
    to?: string;
    category?: string;
  }) {
  ...
  async getOperationalEvents(
    ...
    const limit = Math.min(
      Math.max(Number.parseInt(filters.limit || '100', 10) || 100, 1),
      500,
    );
    const from = this.optionalDate(filters.from, 'from');
    const to = this.optionalDate(filters.to, 'to', true);
    ...
    const [integrationLogs, webhookLogs, actionEvents, providers] =
      await Promise.all([
        !requestedSource || requestedSource === 'INTEGRATION'
          ? prisma.integrationLog.findMany({
              where: {
          ...
          : [],
        !requestedSource || requestedSource === 'WEBHOOK'
          ? prisma.webhookLog.findMany({
              where: {
                ...(providerSlug ? { provider: { slug: providerSlug } } : {}),
                ...(createdAt ? { createdAt } : {}),
          ...
          : [],
        !requestedSource || requestedSource === 'ACTION'
              ...
              where: {
                ...(filters.actionId
                  ? {
                      action: {
                        OR: [
                          { publicId: filters.actionId },
                          { id: filters.actionId },
                          { externalActionId: filters.actionId },
                        ],
                      },
                    }
                  : {}),
          ...
          : [],
        !requestedSource || requestedSource === 'MODERATION'
    ...
    const events: any[] = [
      ...integrationLogs.map((log) => ({
        id: `integration:${log.id}`,
        source: 'INTEGRATION',
        eventType: `${log.method} ${log.endpoint}`,
        severity:
          log.errorMessage || log.statusCode >= 500
            ? 'ERROR'
            : log.statusCode >= 400
              ? 'WARN'
              : 'INFO',
        ...
        durationMs: log.durationMs,
        message: redactForLogs(
          log.errorMessage || 'Provider integration request completed',
        ),
        details: redactForLogs({
      ...
      ...webhookLogs.map((log) => ({
        ...
        eventType: log.event,
        severity:
          log.errorMessage || !log.isVerified
            ? 'ERROR'
            : !log.isProcessed
              ? 'WARN'
              : 'INFO',
        providerSlug: log.provider.slug,
        providerName: log.provider.name,
        actionId: this.extractActionId(log.payload),
        traceId: null,
        status: log.isVerified
          ? log.isProcessed
            ? 'PROCESSED'
            : 'UNPROCESSED'
          : 'INVALID_SIGNATURE',
        ...
        details: redactForLogs(log.payload),
        createdAt: log.createdAt,
      })),
        ...
        severity:
          event.status === 'FAILED'
            ...
            : event.status === 'CANCELLED'
              ? 'WARN'
              : 'INFO',
        providerSlug: event.action.provider.slug,
        providerName: event.action.provider.name,
        actionId: event.action.publicId,
        traceId: null,
        status: event.status,
        durationMs: null,
        message: redactForLogs(event.description),
        details: redactForLogs(event.payload),
        createdAt: event.createdAt,
      })),
      ...providers.flatMap((provider) => {
        const metadata = (provider.metadata as Record<string, any>) || {};
        return (
          Array.isArray(metadata.reviewHistory) ? metadata.reviewHistory : []
        ).map((entry: any, index: number) => ({
          id: `moderation:${provider.slug}:${entry.reviewedAt || index}`,
          source: 'MODERATION',
          eventType: `PROVIDER_${entry.decision}`,
          severity: entry.decision === 'REQUEST_CHANGES' ? 'WARN' : 'ERROR',
          providerSlug: provider.slug,
          providerName: provider.name,
          actionId: null,
          traceId: null,
          status: entry.reasonCode || entry.decision,
          durationMs: null,
          message: redactForLogs(entry.reason),
          details: redactForLogs({
    ...
    const filtered = events
      .filter((event) => {
        ...
        if (filters.actionId && event.actionId !== filters.actionId)
          return false;
  ...
  async exportOperationalEvents(
    filters: Parameters<AdminService['getOperationalEvents']>[0],
    format: 'json' | 'csv',
  ) {
    const safeFilters = filters || {};
    const report = await this.getOperationalEvents({
      ...safeFilters,
    ...
    return [
      columns.join(','),
      ...report.data.map((event: any) =>
        columns.map((column) => escape(event[column])).join(','),
      ),
    ].join('\r\n');
  }
